...
- **Cherry-pick success latency** — successful cherry-pick notifications are now shown immediately after Git finishes applying the commit, before the heavier full state refresh completes. This avoids multi-second delayed success toasts on Windows while still refreshing the extension state afterward.
